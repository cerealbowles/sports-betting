#!/usr/bin/env python3
"""
nfl_bootstrap.py — Calibrate the NFL model against 2024+2025 regular-season data.

Usage:
    python3 nfl_bootstrap.py               # fetch all seasons, run refit
    python3 nfl_bootstrap.py --refit-only  # skip fetch, use cached data

Fetches completed NFL regular-season game results from ESPN, computes season-level
team stats and per-game rest/form from scores, scores each game with
nfl_model.predict(), and runs logistic regression to suggest coefficient updates.

NOTE: NFL has ~272 games/season vs ~2,430 in MLB — coefficients will be noisier.
      Validate live predictions on the Model Performance page before adjusting weights.
"""
...
# No custom User-Agent — Akamai (ESPN's edge/WAF) was returning 403 Forbidden
# for the self-identifying "compatible; nfl-bootstrap/1.0" string, from a
# production IP where nfl_api.py's live schedule fetch — which sends no
# custom header at all, just requests' own default — already hits this exact
# same endpoint successfully. Matching that working, unmodified default was
# the fix; a fake browser UA is exactly the kind of thing bot-detection
# flags, whereas no override at all reads as an ordinary script/library.
...
def fetch_season_games(season):
    """Fetch all completed NFL regular-season games (weeks 1-18) from ESPN."""
    games = []
    print(f'  Fetching {season} schedule (weeks 1-18)...', flush=True)
    for week in range(1, 19):
        ...
        # — `dates` is what actually selects the year (confirmed by
        # inspection: season=2024 returned events dated 2026). Without
        # `dates`, this fetch looked like it worked (200 OK, valid JSON) but
        # returned zero *completed* games for any past season, since it was
        # really fetching the not-yet-played current season.
        r = _get(ESPN_NFL,
...
def refit(training_rows):
    ...
    print('\n' + '═' * W)
    print(f'  NFL Model Refit  —  {seasons_str}  —  {len(training_rows):,} total games')
    print('═' * W)
    ...
    # Bye/short week are rare (~32 bye-week games/season) — bootstrap coeff is valid
    ...
    print('─' * W)
    print()
    print('  NOTE: NFL has ~272 games/season vs ~2,430 in MLB — noisier coefficients.')
    print('  In-sample refit is optimistic. Validate on the Model Performance page.')
    print('═' * W)
    ...
    out_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'nfl_refit_results.json')
    with open(out_path, 'w') as f:
        json.dump(out, f, indent=2)
    print(f'  Full results → nfl_refit_results.json')
    print()
...
def main():
    ...
    print('═' * 62)
    print(f'  NFL Model Bootstrap — {seasons_str}')
    print('═' * 62)
    print()
    ...
    for season in SEASONS:
        ...
        if os.path.exists(cache_path):
            ...
            print(f'  Loaded from cache: {len(games):,} games  '
                  f'(delete nfl_training_{season}.json to re-fetch)')
        else:
            print(f'\n── Season {season} ──────────────────────────────────────')
            ...
            print(f'  Cached → nfl_training_{season}.json')

        rows = build_season_rows(season, games, nfl_model)
    ...
    combined_path = os.path.join(os.path.dirname(os.path.abspath(__file__)),
                                 'nfl_training_combined.json')
    with open(combined_path, 'w') as f:
        json.dump(all_rows, f)
    print(f'\n  Combined: {len(all_rows):,} total rows → nfl_training_combined.json')
