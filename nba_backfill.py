#!/usr/bin/env python3
"""
nba_backfill.py — Populate game_predictions with completed NBA regular-season
games so the Model Performance and History pages have real NBA data, the
same way nfl_backfill.py does for NFL.

Usage:
    python3 nba_backfill.py                    # 2024, 2025 seasons (default)
    python3 nba_backfill.py --dry-run
    python3 nba_backfill.py --seasons 2024 2025

Fetches completed regular-season games from ESPN and, for any game not
already in game_predictions, scores it with nba_model.predict() using a
point-in-time snapshot — each team's win/loss record, home/road split, PPG,
PPG-allowed, last-5 form, and rest days, computed ONLY from games that team
...
class of bug nfl_backfill.py's docstring warns about. nba_bootstrap.py
(coefficient calibration only, not DB writes) has this leak too; fine for a
rough "does the model roughly work" gut check but not for real per-game
predictions on the Model Performance page, so this script computes
wins/losses/split/PPG/PPG-allowed itself, walking each season chronologically.
...
from nba_bootstrap import fetch_season_games, build_recent_form, build_rest_days_map
import nba_model
...
def run(dry_run=False, seasons=None):
    ...
    print('─' * 58)
    print(f'  NBA Backfill — seasons {seasons}{"  (dry run — no writes)" if dry_run else ""}')
    print('─' * 58)
    ...
    # Write to DB — same DISABLE_STARTUP_TASKS guard as nfl_backfill.py, so
    ...
    print(f'    Pick accuracy across {total_resolved} resolved NBA predictions: {acc:.1f}%')
    print(f'\n  Model Performance page now has {total_resolved} resolved NBA predictions.')
    print(f'  Visit /model?sport=NBA to see calibration and factor correlation.')
