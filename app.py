...
def _inject_nav_context():
    """Exposes nav_last_sport — which sport's schedule the header sport-switcher,
    bottom-nav "Today", and "Model" destinations should point at. Backed by a
    plain (unsigned, non-sensitive) `last_sport` cookie set by each sport
    schedule route on render (see _set_last_sport_cookie), since sport is a
