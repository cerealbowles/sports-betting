...
def _get_prior_season_team_stats(season):
    ...
    key = f'wnba_prior_stats_{season}'
    now_ts = time.time()
    if key in _cache:
        data, ts = _cache[key]
...
def _apply_prior_season_blend(team, prior_stats):
    ...
    it accumulates. Sets 'blend_*' keys consumed by wnba_model.predict();
...
def _build_game(event, team_stats, game_log, wnba_odds_map, prior_stats=None):
    ...
    # same fix as nfl_api.py/cfb_api.py/nba_api.py's total_inputs blocks.
    ...
    try:
        total_model = bball_total_model.predict_total(
            'WNBA', home.get('id'), home_ppg, home_ppg_allowed,
