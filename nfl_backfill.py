#!/usr/bin/env python3
"""
nfl_backfill.py — Populate game_predictions with completed NFL regular-season
games so the Model Performance and History pages have real NFL data, the
same way backfill_2026.py does for MLB.

Usage:
    python3 nfl_backfill.py                    # 2024, 2025, 2026 (default)
    python3 nfl_backfill.py --dry-run
    python3 nfl_backfill.py --seasons 2024 2025

Fetches completed regular-season games from ESPN and, for any game not
already in game_predictions, scores it with nfl_model.predict() using a
point-in-time snapshot — each team's win/loss record, home/road split, PPG,
PPG-allowed, last-3 form, and rest days, computed ONLY from games that team
played EARLIER in that same season (season records reset every year).
...
being predicted into the win-percentage and home/road-split factors. That's
the same class of bug backfill_2026.py's docstring warns about for MLB.
nfl_bootstrap.py (coefficient calibration only, not DB writes) has this
leak too; it's a look-ahead-biased proxy that's fine for a rough "does the
model roughly work" gut check but not for real per-game predictions on the
Model Performance page, so this script computes wins/losses/split/PPG/PPG-
allowed itself, walking each season chronologically (see
build_pointintime_stats below). Recent form and rest days are pulled
straight from nfl_bootstrap.py, which already gets those right (they
already filter to games strictly before the one being scored).
...
with no completed games yet just contribute zero rows.
"""
import json
import os
import sys
...
from nfl_bootstrap import fetch_season_games, build_recent_form, build_rest_days_map
import nfl_model
...
def build_pointintime_stats(season_games):
    ...
    ppg_allowed}}. Assumes at most one game per team per date (true for the
    NFL regular season).
    """
...
def run(dry_run=False, seasons=None):
    ...
    print('─' * 58)
    print(f'  NFL Backfill — seasons {seasons}{"  (dry run — no writes)" if dry_run else ""}')
    print('─' * 58)
    ...
    # Write to DB — same DISABLE_STARTUP_TASKS guard as backfill_2026.py, so
    # importing app.py here doesn't start the scheduler or warm live caches.
    ...
    with flask_app.app_context():
        existing_keys = set(
            ...
            ).filter_by(sport='NFL').all()
        )

        for r in rows:
            try:
                key = (r['game_date'], r['home_name'], r['away_name'])
    ...
    print(f'    Pick accuracy across {total_resolved} resolved NFL predictions: {acc:.1f}%')
    print(f'\n  Model Performance page now has {total_resolved} resolved NFL predictions.')
    print(f'  Visit /model?sport=NFL to see calibration and factor correlation.')
